...
def init():
    ...
    driver_wait = WebDriverWait(driver, 10) # Timeout
    return driver, driver_wait


def login(driver: webdriver.Chrome):
    logging.info("Attempt to login to SSO")
...
def get_timetable_page(driver: webdriver.Chrome, driver_wait: WebDriverWait):
    logging.info("Switch to the timetable page")
    driver_wait.until(ec.visibility_of_element_located((By.CLASS_NAME, "custom-link-container"))).click()
    driver.switch_to.window(driver.window_handles[1])


def get_screenshot(driver: webdriver.Chrome):
    logging.info("Loading of the timetable page according to the date defined")
